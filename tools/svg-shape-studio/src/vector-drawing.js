export function finishDrawnVector(item, closed = false) {
  ...
  return {...item,x:item.x+minX,y:item.y+minY,width:Math.max(.01,maxX-minX),height:Math.max(.01,maxY-minY),nodes:item.nodes.map(node=>({...node,x:node.x-minX,y:node.y-minY})),open,collision:!open,fill:open?'none':item.fill==='none'?'#e9f5bc':item.fill};
}
