...
// Kept temporarily for migrating older saved stageplots; no longer shown in the library.
// eslint-disable-next-line no-unused-vars
const TOOLS = [
  { type: 'drums', label: 'Drum kit', icon: '◉', tone: 'amber' },
  { type: 'keys', label: 'Keyboard', icon: '▤', tone: 'blue' },
  { type: 'guitar', label: 'Guitar amp', icon: '▥', tone: 'violet' },
  { type: 'bass', label: 'Bass amp', icon: '▦', tone: 'green' },
  { type: 'mic', label: 'Vocal mic', icon: '●', tone: 'rose' },
  { type: 'monitor', label: 'Monitor', icon: '◢', tone: 'slate' },
  { type: 'stool', label: 'Stool', icon: '○', tone: 'orange' },
  { type: 'power', label: 'Power', icon: 'ϟ', tone: 'yellow' },
]

const INITIAL_ITEMS = [
  { id: 1, type: 'drums', label: 'Drum kit', icon: '◉', tone: 'amber', x: 43, y: 24 },
  { id: 2, type: 'keys', label: 'Keyboard', icon: '▤', tone: 'blue', x: 17, y: 31 },
  { id: 3, type: 'guitar', label: 'Guitar amp', icon: '▥', tone: 'violet', x: 70, y: 32 },
  { id: 4, type: 'mic', label: 'Lead vocal', icon: '●', tone: 'rose', x: 44, y: 66 },
  { id: 5, type: 'monitor', label: 'Monitor', icon: '◢', tone: 'slate', x: 30, y: 72 },
  { id: 6, type: 'monitor', label: 'Monitor', icon: '◢', tone: 'slate', x: 59, y: 72 },
]
...
const zoneSegmentMode = (node) => node.curveMode || (node.curve ? 'smooth' : 'line')
const zoneSegmentPath = (from, to) => {
  if (zoneSegmentMode(to) === 'pointArc') { const arc = { x: to.arcX ?? (from.x + to.x) / 2, y: to.arcY ?? (from.y + to.y) / 2 }; const control = { x: arc.x * 2 - (from.x + to.x) / 2, y: arc.y * 2 - (from.y + to.y) / 2 }; return `Q ${control.x} ${control.y} ${to.x} ${to.y}` }
  if (zoneSegmentMode(to) === 'smooth') { const midpoint = { x: (from.x + to.x) / 2, y: (from.y + to.y) / 2 }; const length = Math.hypot(to.x - from.x, to.y - from.y) || 1; const control = Number.isFinite(to.bulge) ? { x: midpoint.x - (to.y - from.y) / length * to.bulge, y: midpoint.y + (to.x - from.x) / length * to.bulge } : { x: to.cx ?? midpoint.x, y: to.cy ?? midpoint.y }; return `Q ${control.x} ${control.y} ${to.x} ${to.y}` }
  if (zoneSegmentMode(to) === 'bezier') { const midpoint = { x: to.mx ?? (from.x + to.x) / 2, y: to.my ?? (from.y + to.y) / 2 }; return `C ${to.startOutX ?? (from.x * 2 + midpoint.x) / 3} ${to.startOutY ?? (from.y * 2 + midpoint.y) / 3} ${to.midInX ?? (from.x + midpoint.x * 2) / 3} ${to.midInY ?? (from.y + midpoint.y * 2) / 3} ${midpoint.x} ${midpoint.y} C ${to.midOutX ?? (midpoint.x * 2 + to.x) / 3} ${to.midOutY ?? (midpoint.y * 2 + to.y) / 3} ${to.endInX ?? (midpoint.x + to.x * 2) / 3} ${to.endInY ?? (midpoint.y + to.y * 2) / 3} ${to.x} ${to.y}` }
  return `L ${to.x} ${to.y}`
}
const zonePath = (nodes) => nodes?.length >= 3 ? `M ${nodes[0].x} ${nodes[0].y} ${nodes.slice(1).map((node, index) => zoneSegmentPath(nodes[index], node)).join(' ')} ${zoneSegmentPath(nodes.at(-1), nodes[0])} Z` : ''
...
function VectorShape({ shape }) {
  ...
  let node
  if (shape.type === 'circle' || shape.type === 'ellipse') node = <ellipse cx={shape.width / 2} cy={shape.height / 2} rx={shape.width / 2} ry={shape.height / 2} {...style} />
...
function App() {
  ...
  const [items, setItems] = useState(INITIAL_ITEMS.slice(0, 0))
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        ...
        if (active) { setLibrary(data); setGroups(groupData); setStages(stageData); setLibraryOnline(true) }
      ...
      <main className="workspace">
        <aside className="sidebar library-sidebar">
          <section className="library">
            ...
            <p className="helper">Click an item to add it to the stage.</p>
            {library.length > 0 && <div className="grouped-library">
              {[...groups, { id: null, label: 'Uncategorised' }].map((group) => {
                const tools = library.filter((tool) => (tool.groupId || null) === group.id)
                return tools.length ? <div className="equipment-group" key={group.id || 'none'}><p>{group.label}</p><div className="custom-tools">{tools.map((tool) => <button className="custom-tool" key={tool.id} draggable onDragStart={(event) => { event.dataTransfer.effectAllowed = 'copy'; event.dataTransfer.setData('application/x-stageplot-item', tool.id) }} onClick={() => addItem({ ...tool, type: `library:${tool.id}`, tone: 'custom' })}>
                  <span><VectorArtwork shapes={tool.shapes} width={tool.dimensions.widthMeters} depth={tool.dimensions.depthMeters} /></span><span><b>{tool.label}</b><small>{tool.dimensions.widthMeters} × {tool.dimensions.depthMeters} m</small></span>
                </button>)}</div></div> : null
              })}
            </div>}
          </section>

          <div className="sidebar-tip"><span>i</span><p><strong>Quick tip</strong>Double-click a placed label to rename it.</p></div>
        </aside>
        ...
        <section className="canvas-area">
          ...
          <div className="stage-viewport" ref={viewportRef} onWheel={zoomViewport} onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = 'copy' }} onDrop={dropLibraryItem} onPointerDown={startPan} onPointerMove={moveViewport} onPointerUp={() => { dragRef.current = null; rotateRef.current = null; panRef.current = null }} onPointerCancel={() => { dragRef.current = null; rotateRef.current = null; panRef.current = null }}>
            <div className="stage" ref={boardRef} style={{ width: `${space.width * 100}px`, height: `${space.depth * 100}px`, transform: `translate(${pan.x}px, ${pan.y}px) scale(${zoom})` }}>
              ...
              {(space.boundary?.nodes || space.zones?.length > 0) && <svg className="stage-zones" viewBox={`0 0 ${space.width} ${space.depth}`} style={{ clipPath: stageClipPath(space) }}>{(space.zones || []).filter((zone) => !zone.solid && !zone.label).map((zone) => <path key={zone.id} d={zonePath(zone.nodes)} fill={zone.fill} fillOpacity={zone.fillOpacity} stroke={zone.stroke} strokeOpacity={zone.strokeOpacity} strokeWidth={zone.strokeWidth} vectorEffect="non-scaling-stroke" />)}{space.boundary?.nodes && <path className="main-stage-outline" d={zonePath(space.boundary.nodes)} />}{(space.zones || []).filter((zone) => zone.solid).map((zone) => <path className="solid-zone" key={zone.id} d={zonePath(zone.nodes)} />)}</svg>}
              {((space.zones || []).some((zone) => zone.label) || space.textItems?.length > 0) && <svg className="stage-label-assets" viewBox={`0 0 ${space.width} ${space.depth}`}>{(space.zones || []).filter((zone) => zone.label).map((zone) => <path key={zone.id} d={zonePath(zone.nodes)} fill={zone.fill} fillOpacity={zone.fillOpacity} stroke={zone.stroke} strokeOpacity={zone.strokeOpacity} strokeWidth={zone.strokeWidth} vectorEffect="non-scaling-stroke" />)}{(space.textItems || []).map((item) => <text key={item.id} x={item.x} y={item.y} fill={item.color} fillOpacity={item.opacity} fontSize={item.fontSize} textAnchor="middle" dominantBaseline="middle">{item.text}</text>)}</svg>}
              ...
              {items.map((item) => (
                ...
                >
                  ...
                  <span className="placed-label">{item.label}</span>
